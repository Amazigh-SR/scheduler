...
describe("Application", () => {
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 5. Change student name to "Tom Holland".
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
      target: { value: "Tom Holland" },
    });
    // 6. Click the "Save" button on that same appointment.
    fireEvent.click(getByText(appointment, "Save"));
    // 7. Check that the element with the text "Saving" is displayed.
    expect(getByText(appointment, "Saving")).toBeInTheDocument();
    // 8. Wait until the element with the text "Lydia Miller-Jones" is displayed.
    await waitForElement(() => getByText(appointment, "Tom Holland"));
  ...
  it("shows the save error when failing to save an appointment", async () => {
    ...
    // 7. Check that the element with the text "Saving" is displayed.
    expect(getByText(appointment, "Saving")).toBeInTheDocument();
    // 8. Check that "Error, could not save the appointment" is present in the DOM
  ...
  it("shows the delete error when failing to delete an existing appointment", async () => {
    ...
    // 6. Check that the element with the text "Deleting" is displayed.
    expect(getByText(appointment, "Deleting")).toBeInTheDocument();
    // 7. Wait until the element with the "Error, could not cancel the appointment" button is displayed.
    await waitForElement(() =>
